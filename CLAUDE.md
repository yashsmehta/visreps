...
├── figures/               # Output PNGs generated by plotting scripts
├── plotter_utils.py       # Shared helpers (best-layer selection, bar plot)
├── bar_plot_comparison.py # RSA / semantic bar plots (MODE toggle)
├── box_plot_architectures.py  # RSA / encoding box plots (METRIC toggle)
├── paired_comparison.py   # Coarse-vs-fine / architecture paired plots (MODE toggle)
├── coarseness_barplot.py  # Granularity bar + axis-break (SQLite)
├── rsa_score_by_classes.py
├── encoding_score_by_classes.py
└── tvsd_coarse_vs_fine_barplots.py
...
Plotting scripts live in `plotters/`, outputs go to `plotters/figures/`. Three consolidated scripts use a MODE/METRIC constant at the top to toggle behavior: `bar_plot_comparison.py` (rsa/semantic), `box_plot_architectures.py` (rsa/encoding), `paired_comparison.py` (coarse_vs_fine/architectures). Stimulus robustness and sensitivity analyses are self-contained in `experiments/stimulus_robustness/` and `experiments/stimulus_sensitivity/`.
