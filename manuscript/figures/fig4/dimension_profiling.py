"""
Figure 4 Panel C: THINGS Semantic Dimension Profiling.
...
Run from project root:
    python manuscript/figures/fig4/dimension_profiling.py
"""
...
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "dimension_profiling.png")
PROTOTYPE_PATH = os.path.join(OUTPUT_DIR, "dimension_alignment_prototype.png")
...
def compute_per_concept_diff(data):
    """Compute per-concept RDM row correlation difference (CLIP 8-class - 1K)."""
...
def main():
    ...
    print(f"  CLIP-8 wins on {(diff > 0).sum()}/{len(diff)} concepts")
    ...
    # Sort by rho value: most positive on top, most negative on bottom.
    # In barh, highest y-index is drawn at the top, so sort ascending
    # (smallest rho at index 0 = bottom, largest at index N = top).
    order = np.argsort(np.abs(rhos))[::-1][:TOP_N]  # top N by |rho|
    order = order[np.argsort(rhos[order])]  # ascending rho → top bar is most positive
    ...
    ax.set_xlabel(r"$\Delta\rho_s$ (CLIP 8-class $-$ 1000-class)", fontsize=8.5)
    ax.axvline(0, color="black", lw=0.8, zorder=4)
    ax.grid(axis="x", alpha=0.25, zorder=0)

    # Symmetric x-axis
    x_max = max(abs(rhos[order].min()), abs(rhos[order].max())) * 1.08
    ...
    ax.text(0.98, 0.03,
            ...
            f"Green = high loading favors 8-class model\n"
    ...
    fig.savefig(PROTOTYPE_PATH, dpi=300, bbox_inches="tight", facecolor="white")
    plt.close(fig)
