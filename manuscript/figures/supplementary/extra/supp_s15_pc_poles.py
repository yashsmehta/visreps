"""Supplementary Figure S15: PC Axis Interpretation.
...
# ── Config ────────────────────────────────────────────────────────────────
OUTPUT = "manuscript/figures/supplementary/supp_s15_pc_poles.png"
...
def load_model_data(model_key: str, imagenet_dir: str):
    ...
    Returns:
        all_high: dict[pc -> list[PIL.Image]]
        all_low: dict[pc -> list[PIL.Image]]
        var_explained: list[float] (percent variance for each PC)
    """
...
def make_figure(model_data: dict) -> plt.Figure:
    ...
    Each model block: 6 rows (PCs) x (5 high + gap + 5 low) columns.
    Two blocks separated by a wider gap.
    """
    n_models = len(model_data)
    n_cols_per_block = 2 * N_PER_POLE + 1  # 5 high + 1 gap + 5 low = 11

    cell_size = 0.75  # inches per image cell
    gap_ratio = 0.3   # gap column width relative to image column
    model_gap = 1.0   # inches between model blocks
    left_margin = 0.9  # inches for PC labels
    top_margin = 0.7   # inches for headers
    bottom_margin = 0.1

    # Width ratios for one block: [1]*5 + [gap] + [1]*5
    block_ratios = [1.0] * N_PER_POLE + [gap_ratio] + [1.0] * N_PER_POLE
    # Full width ratios: block1 + model_gap_col + block2
    ...
    for model_idx, model_key in enumerate(model_keys):
        ...
        # Column offset for this model block
        col_offset = model_idx * (n_cols_per_block + 1)  # +1 for gap column
        ...
        # Sub-headers for most/least activating
