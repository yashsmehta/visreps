"""Figure 3 Variant: Proportional fill columns integrated into plot area.

Instead of dot grids below the x-axis, this variant draws subtle vertical
background columns whose fill height is proportional to n_classes/1000.
Coarse models show tiny slivers; 1000-way fills the entire column —
dramatically visualizing how little supervision is needed.

Usage:
    python manuscript/figures/fig3/figure3_variant_integrated.py
"""
...
# ── Proportional fill columns ─────────────────────────────────────────

def draw_proportional_fills(ax):
    """Draw subtle vertical fill columns proportional to n_classes/1000.

    Each column spans from y_min up to y_min + (y_max - y_min) * n/1000.
    Must be called AFTER data is plotted so y-limits are finalized.
    """
    y_min, y_max = ax.get_ylim()
    y_range = y_max - y_min

    # Two-tone approach: light outline for the full column, colored fill for proportion
    outline_color = "#edf1f5"
    outline_alpha = 0.45
    fill_color = "#7da8c4"
    fill_alpha = 0.28

    class_counts = COARSE_CFGS + [1000]
    x_positions = COARSE_CFGS + [BREAK_1K_POS]

    for n_classes, x_pos in zip(class_counts, x_positions):
        # Fill height: linear proportion of n_classes / 1000
        fill_height = y_min + y_range * (n_classes / 1000.0)

        # Column width in log space — multiplicative factor for consistent visual width
        x_lo = x_pos / 1.30
        x_hi = x_pos * 1.30

        # Light ghost column showing full potential height (subtle context)
        ax.fill_between(
            [x_lo, x_hi], y_min, y_max,
            color=outline_color, alpha=outline_alpha,
            edgecolor="none", zorder=0,
        )

        # Filled portion: proportional to n_classes/1000
        ax.fill_between(
            [x_lo, x_hi], y_min, fill_height,
            color=fill_color, alpha=fill_alpha,
            edgecolor="none", zorder=0,
        )

    # Restore original y-limits (fill_between can change them)
...
def plot_raw_coarseness(ax, dataset, region, show_ylabel=True, show_xlabel=True):
    ...
    # Architectures (AlexNet, CLIP, Pixels)
    ...
    # Draw proportional fills AFTER axes are formatted (y-limits finalized)
    draw_proportional_fills(ax)
...
def main():
    ...
    # 3 rows (TVSD, separator, NSD) x 2 regions — more compact bottom margin
    ...
    # ── Save ──
    out = f"{OUTPUT_DIR}/figure3_variant_integrated.png"
