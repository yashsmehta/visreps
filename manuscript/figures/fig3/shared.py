...
Used by panel_raw.py, panel_efficiency.py, and figure3.py.
"""
...
def fetch_arch_data(dataset, folder, region):
    ...
    NSD uses bootstrap CIs; TVSD uses 1.96 * SEM across seeds.
    """
    if dataset == "nsd":
        means, ci_lo, ci_hi = [], [], []
        ...
        for cfg in COARSE_CFGS:
            ...
            sems.append(sem)
        means = np.array(means)
        errs_lo = errs_hi = 1.96 * np.array(sems)
...
def format_yaxis(ax, fmt_str=".2f"):
