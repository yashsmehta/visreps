"""Figure 3 Variant: Proportional pillars below x-axis.

Same as figure3.py but replaces dot-grid class count visualization with
proportional fill pillars. Each pillar has the same total height (representing
1000 classes) and is filled from the bottom proportionally to the class count.

Uses log-scale fill for visual readability (linear 2/1000 = 0.2% is invisible).

Usage:
    python manuscript/figures/fig3/figure3_variant_pillars.py
"""
...
# ── Class count proportional pillars ─────────────────────────────────────

def draw_class_count_pillars(fig, bottom_axes):
    """Draw proportional fill pillars below x-axis ticks to visualize class counts.

    Each pillar has the same total height (representing 1000 classes). The filled
    portion uses a log-scale mapping: fill_fraction = log(n) / log(1000), which
    makes the difference between small and large counts visually dramatic while
    keeping even 2-class visible.
    """
    ...
    fill_color = '#8fa8be'    # muted slate blue
    bg_color = '#f0f2f5'      # very light background for unfilled portion
    border_color = '#b5b5b5'  # thin border

    pillar_width = 0.026   # figure fraction
    pillar_height = 0.058  # figure fraction (total height of each pillar)
    gap = 0.018            # gap below axes y0
    ...
    for ax in bottom_axes:
        ...
        for x_data, n_classes in zip(x_data_positions, class_counts):
            ...
            left = cx - pillar_width / 2
            bottom = ax_pos.y0 - gap - pillar_height

            if bottom < 0.01 or left < 0:
                continue

            # Log-scale fill fraction: log(n)/log(1000)
            # This gives: 2->0.10, 4->0.20, 8->0.30, 16->0.40, 32->0.50, 64->0.60, 1000->1.0
            fill_frac = np.log(n_classes) / np.log(1000)

            # Background pillar (full height, light color)
            inset = fig.add_axes([left, bottom, pillar_width, pillar_height])
            inset.set_xlim(0, 1)
            inset.set_ylim(0, 1)
            inset.set_facecolor(bg_color)

            # Filled portion from bottom
            inset.bar(0.5, fill_frac, width=0.98, bottom=0,
                      color=fill_color, edgecolor='none', zorder=2)
            ...
            # Class count label below pillar
            fig.text(cx, bottom - 0.004, str(n_classes),
                     fontsize=6.5, ha="center", va="top", color="#444444")
...
def main():
    ...
    # ── Remove "Classes" xlabel and x-tick labels from bottom row — pillars replace them ──
    axes[(2, 0)].set_xlabel("")
    axes[(2, 1)].set_xlabel("")
    axes[(2, 0)].set_xticklabels([])
    axes[(2, 1)].set_xticklabels([])

    # ── Class count proportional pillars (below bottom row) ──
    draw_class_count_pillars(fig, [axes[(2, 0)], axes[(2, 1)]])

    # ── Per-axis "Number of training classes" labels ──
    for ax in [axes[(2, 0)], axes[(2, 1)]]:
        ax_pos = ax.get_position()
        ax_cx = (ax_pos.x0 + ax_pos.x1) / 2
        label_y = ax_pos.y0 - 0.018 - 0.058 - 0.004 - 0.020
        fig.text(ax_cx, max(label_y, 0.035),
    ...
    # ── Save ──
    out = f"{OUTPUT_DIR}/figure3_variant_pillars.png"
