...
def test_model_weights_different():
    ...
    base_cfg = {
        ...
        'exp_name': 'seed_test',
        'checkpoint_model': 'checkpoint_epoch_0.pth'
    }
    ...
    model1 = load_model(cfg1, device, num_classes=200)
    model2 = load_model(cfg2, device, num_classes=200)
    ...
    for (name1, p1), (name2, p2) in zip(model1.named_parameters(), model2.named_parameters()):
        ...
        if p1.shape != p2.shape:
            shape_mismatches.append((name1, p1.shape, p2.shape))
            continue
    ...
    # Print results
    if shape_mismatches:
        print("\nParameters with shape mismatches:")
        for name, shape1, shape2 in shape_mismatches:
            print(f"{name}: Model 1 shape {shape1} vs Model 2 shape {shape2}")
            
    if param_diffs:
        print("\nParameters with different weights (same shape):")
        for name, diff in param_diffs:
            print(f"{name}: Mean absolute difference = {diff:.6f}")
    
    if not shape_mismatches and not param_diffs:
        print("\nWARNING: All model parameters are identical!")
