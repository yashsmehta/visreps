...
from bonner.datasets.hebart2022_things_behavior._data import load_embeddings as _load_things_embed
...
def load_nsd_data(cfg: Dict) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    ...
    images        = {str(k): v for k, v in utils.load_pickle(os.path.join(root, "stimuli.pkl")).items()}
    ids           = {str(i) for i in fmri_xr.coords["stimulus"].values} & images.keys()
    return ({i: fmri_xr.sel(stimulus=int(i)).values for i in ids},
...
def load_things_data() -> tuple[dict[str, np.ndarray], dict[str, str]]:
    ...
    embeds = {str(i): beh_xr.sel(object=i).values.astype(np.float32)
    ...
    for k in stimset.metadata.index:
        try:
            ...
            correct_path = os.path.join(stimset.root, "images", "object_images", relative_path_part)
            if os.path.exists(correct_path):
                img_paths[k] = correct_path
            else:
                logger.warning(f"Constructed path {correct_path} for image {k} does not exist. Skipping.")
        except KeyError:
            logger.error(f"Key {k} not found in _ThingsStimSet metadata. Skipping.")
        except Exception as e:
            logger.warning(f"Error constructing path for image {k}: {e}. Skipping.", exc_info=False)
    ...
    def _load_and_transform(self, data_or_path: Any, key: str):
        ...
        try:
            if isinstance(data_or_path, str):
                ...
                logger.warning(f"Unexpected data type {type(data_or_path)} for key {key}. Returning placeholder.")
            ...
            logger.warning(f"Image file not found for key {key} at path {path_used}. Returning placeholder.")
        except OSError as e:
            err_loc = f"at path {path_used}" if path_used else "from data"
            if "truncated" in str(e):
                logger.warning(f"OSError (truncated) processing image key {key} {err_loc}. Applying transform to placeholder.")
            else:
                logger.error(f"Unhandled OSError for key {key} {err_loc}: {e}. Returning placeholder.")
        except Exception as e:
            err_loc = f"at path {path_used}" if path_used else "from data"
            logger.error(f"Unexpected error for key {key} {err_loc}: {e}. Returning placeholder.")
        ...
        try:
            ...
            logger.error(f"Failed to create/transform placeholder for key {key}: {placeholder_e}")
...
def custom_collate_fn(batch: List[Tuple[torch.Tensor, str]]
...
def get_neural_loader(cfg: Dict) -> Tuple[Dict[str, Any], DataLoader]:
    ...
    if dataset_type == "nsd":
        ...
        raise ValueError("neural_dataset must be 'nsd' or 'things'")
